...
public class HttpRequestUtilTest {
    ...
    void readValidFile() throws IOException {
        InputStream is = new ByteArrayInputStream("GET /index.html HTTP/1.1\nHost: localhost:8080\nConnection: keep-alive\nAccept: */*".getBytes());
        String path = HttpRequestUtil.getPath(new BufferedReader(new InputStreamReader(is, StandardCharsets.UTF_8)).readLine());

        String s = new String(HttpRequestUtil.getBody(path), StandardCharsets.UTF_8);
